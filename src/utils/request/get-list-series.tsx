...
async function GetListSeries(offset: Number): Promise<InterfaceReturn[]> {
	...
	try {
		const response = await ApiMarvelPublic.get(`series?orderBy=title&limit=20&offset=${offset}&ts=${time}&apikey=${apiKeyPublic}&hash=${hash}`)
