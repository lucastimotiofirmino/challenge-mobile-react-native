...
async function GetListCharactes(offset: Number): Promise<InterfaceReturn[]> {
	...
	try {
		const response = await ApiMarvelPublic.get(`characters?orderBy=name&limit=20&offset=${offset}&ts=${time}&apikey=${apiKeyPublic}&hash=${hash}`)
...
export default GetListCharactes