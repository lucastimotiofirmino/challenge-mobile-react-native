...
export function* fetchCharacters() {
   try {
      ...
      yield put(charactersActions.entity.add(characters))
...
export function* fetchCharactersByName() {
   try {
      const { characterName } = yield select(state => state.ui.characters)

      const response: AxiosResponse = yield call(
         () => api.get(`/characters?orderBy=name&nameStartsWith=${characterName}&limit=100&ts=1&apikey=881c88e27f2ea86fac39d6ee4156fbd1&hash=f1b660c39d1bcd3047d0d241a5b86357`)
      )
